---
title: "Collabathon: Simulating infectious disease data"
...
* We parameterize all distributions (except offspring, which were Poisson for now) using a right-truncated Negative Binomial (NB) distribution. Throughout, NB size is a measure of over-dispersion (smaller size means more over-dispersion, which means variance != mean. Higher size means the distribution gets closer to Poisson). For more information see [Zeileis](https://edoc.unibas.ch/15400/).
...
inputPanel(
  ...
  sliderInput("trans_i_size", label = "NB size", pre = '1e', value = 2, min = -10, max = 20),
...
Now, create your $R(t)$ curve so you can simulate the next generation(s) of cases (**starting at $t=1$**). Pre-defining the shape of $R(t)$ allows you to simulate infectious disease dynamics, interventions, etc. The dotted line at $R(t)=1$ shows the span of $R(t)$, from $t=[1, T]$. $R(t > T)$ are set to 1. if $R(t) > 1$ for all $t$, the cases will exponentially increase, and the code may not complete for modeling windows greater than 30 days. You can change the code to have a different spline between points. 
...
inputPanel(
  ...
  sliderInput("n0", label = "Initial cases:",pre = '1e',
              min = 1, max = 3, value = 3, step = 0.5),
  sliderInput("ld", label = "Modeling window (days)", value = 60,
