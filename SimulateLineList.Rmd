...
```{r, echo = FALSE}
mainPanel(
  downloadButton("downloadSerial", "Serial interval")
)

output$downloadSerial <- downloadHandler(
    filename = function() {
        paste("serial_dist", Sys.Date(), ".csv", sep = "")
    },
    
    content = function(file) {
        write.csv(  as_tibble(serial_data()) %>% cbind(1)) 
    }
)
