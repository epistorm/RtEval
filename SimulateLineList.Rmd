...
knitr::include_graphics("rsif20200756f01.jpg")
```
...
Generation time is the time between **infection** in the **infector** and **infection** in the **infectee**, dervied as $G_{ij} = I_{i} + P_{ij}$ (Eq. 2.1b).
...
The serial interval can derived from the generation time distribution and the incubation time distribution: $S_{ij} = P_{ij} + I_j$ (Eq. 2.1a).
...
rv <- reactiveValues(
  ...
  y = c(1.5, 0.5, 0.6, 0.6, 1.28, 1.5, 1.4, 1.38)
)
...
dat_all <- reactive({
  ...
  while(any(i1<=input$ld)) {
    ...
    rt <- c(rt, rep(input$RTINF, times = max(date))) ## repeat RTINF until the end of  

    ### IF THERE IS A 0 you have to add r0
    if(0 %in% date) {
      rt_vec = c(input$r0, rt[date])
    } else {
      rt_vec = rt[date] 
    }
    
    stopifnot(length(rt_vec) == length(no))
    mu   <- no * rt_vec
    
    ## simulate the next generation of infectees
    n1   <- sapply(mu, function(mx) rpois(1, mx))

    ## and their infection dates
    inf1 <- sample(TRANS_MIN:input$maxtrans, 
                        size = sum(n1), 
                        prob = dnbinom(TRANS_MIN:input$maxtrans, 
                                       size = 1*10^input$trans_i_size, 
                                       mu = input$trans_i_mean), 
    ...
    i1   <- rep(date, times = n1) + inf1
    
    ## and thier onset times
...
output$downloadAggData <- downloadHandler(
    ...
    group_by(name, value) %>% tally() %>% 
      ...
      pivot_wider(id_cols = day, names_from = name,
                  values_from = n, values_fill = 0), file, row.names = F)
    }
)
...
output$dist_rt <- downloadHandler(
    ...
    content = function(file) {
        write.csv(as_tibble(rt_grid()), file, row.names = F)
    }
)
